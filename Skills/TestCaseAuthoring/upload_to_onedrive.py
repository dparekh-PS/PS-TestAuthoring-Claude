...
download URL, which this proxy blocks -- so this script can upload but cannot
verify-by-reading-back in that kind of restricted environment. Metadata reads
(listing children, item properties) stay on graph.microsoft.com and work.
...
def upload_one(token, drive_id, item_id, local_path):
    ...
    j = r.json()
    return {"name": name, "id": j.get("id"), "size": j.get("size"), "webUrl": j.get("webUrl")}
...
def main():
    ...
    for res in results:
        print(f"{res['name']}: uploaded (id={res['id']}, size={res['size']})")
